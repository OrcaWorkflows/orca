...
function isConfGiven(nodes:Elements, index:number):boolean {
    ...
    else if (nodeName.indexOf("Elasticsearch") >= 0 && nodes[index].data.conf.hasOwnProperty("host")) {
        return true;
    }
    else if (nodeName.indexOf("PubSub") >= 0 && nodes[index].data.conf.hasOwnProperty("topic")) {
        return true;
    }
    else if (nodeName.indexOf("BigQuery") >= 0 && nodes[index].data.conf.hasOwnProperty("table_id")) {
        return true;
    }
...
function appendRequiredVariables(nodes:Elements, task: Task, name: string, index: number) {
...
function taskGenerator(nodes:Elements, edge:Edge, dependencies:Array<string>, sourceIndex:number, targetIndex:number) {
...
export function createTaskForEdge(nodes:Elements, edges:Elements, edge:Edge) {
    const sourceIndex = nodes.findIndex((node:FlowElement) => (node as Node).id === edge.source);
    const targetIndex = nodes.findIndex((node:FlowElement) => (node as Node).id === edge.target);
    ...
    State.tasks.push(taskGenerator(nodes, edge, dependencies, sourceIndex, targetIndex));
}
